using System;
using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

namespace ComfyBlazorApp.Migrations
{
    /// <inheritdoc />
    public partial class AddPromptHistory : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "PromptHistories",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "TEXT", nullable: false),
                    Prompt = table.Column<string>(type: "TEXT", maxLength: 4000, nullable: false),
                    NegativePrompt = table.Column<string>(type: "TEXT", maxLength: 4000, nullable: false),
                    Mode = table.Column<string>(type: "TEXT", maxLength: 20, nullable: false, defaultValue: "Generate"),
                    SourceImagePath = table.Column<string>(type: "TEXT", maxLength: 500, nullable: true),
                    BatchSize = table.Column<int>(type: "INTEGER", nullable: false),
                    ComfyPromptId = table.Column<string>(type: "TEXT", maxLength: 100, nullable: true),
                    CreatedAt = table.Column<DateTime>(type: "TEXT", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_PromptHistories", x => x.Id);
                });

            migrationBuilder.CreateIndex(
                name: "IX_PromptHistories_CreatedAt",
                table: "PromptHistories",
                column: "CreatedAt");
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "PromptHistories");
        }
    }
}
